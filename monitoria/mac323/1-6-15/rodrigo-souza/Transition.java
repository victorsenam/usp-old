...
public class Transition {
    ...
    public static void main(String[] args) {
        ...
        int P = StdIn.readInt();           // number of pages
        int H = StdIn.readInt();           // number of Hubs
        int A = StdIn.readInt();           // number of Authorities
        int N = P+H+A;
...
//        for (int k = 0; k < 10; k++)  {
        ...
        for (int i = 0; i < N; i++)  {
            ...
            for (int j = 0; j < N; j++) {
            	double p;
            	// Se a pagina nao tiver links, soma apenas a probabilidade de sair dela
            	if (outDegree[i] != 0)
