...
\begin{homeworkProblem}[(Completo)]
...
\begin{homeworkProblem}[(Itens i e ii)]
...
\begin{proof}[Resposta]
Podemos escrever $\langle A_i, X \rangle = b_i$, $\forall i \in [m]$ como $\mathcal{A}(X) = b$, onde
$\mathcal{A}(X) \in \R^m$  e $\mathcal{A}(X)_i = \langle A_i, X \rangle$ para todo $i \in [m]$. Precisamos montar o dual de $\ref{ex5}$, para isso, adicionamos a variável dual $y \in R^m$. E temos que calcular a transformação $\mathcal(A)^*$, dual de $\mathcal(A)$. Esta deve ser tal que 
$$ \langle X, \mathcal{A}^*(X) \rangle = \langle \mathcal{A}(X), y \rangle \text{, }$$
temos que
$$ \langle \mathcal{A}(X), y \rangle = \mathcal{A}(X)^Ty = \sum \limits_{i=1}^m y_i\mathcal{A}(X)_i = \sum \limits_{i=1}^m y_i \langle A_i, X \rangle = \langle \sum \limits_{i=1}^m y_iA_i, X \rangle \text{, }$$
portanto, podemos usar $\mathcal{A}^*(X) = \sum \limits_{i=1}^m y_iA_i$ (vale lembrar que a ideia desta prova foi vista em aula).

\begin{align} \label{ex5_dual}
\max \quad        &\langle b, y \rangle & \nonumber \\
\text{s.a.} \quad &y \in \R^m \text{, } & \\
                  &\sum \limits_{i=1}^m y_iA_i \in \S_+^n \text{.} & \nonumber
\end{align}

Eu não consegui nada mais do que isso.
\end{proof}
